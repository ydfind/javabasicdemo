...
public class MyHashMap<K,V> extends MyAbstractMap<K,V>
    ...
    final class EntrySet extends AbstractSet<MyMap.MyEntry<K,V>> {
        public final int size()                 { return size; }
        public final void clear()               { MyHashMap.this.clear(); }
    ...
    // Overrides of JDK8 MyMap extension methods
    ...
    public V computeIfAbsent(K key,
        ...
        if (mappingFunction == null)
            throw new NullPointerException();
        int hash = hash(key);
        MyNode<K,V>[] tab; MyNode<K,V> first; int n, i;
        int binCount = 0;
        TreeNode<K,V> t = null;
        MyNode<K,V> old = null;
...
// DRD: 无法找到SharedSecrets，先注释掉
//            SharedSecrets.getJavaOISAccess().checkArray(s, MyMap.MyEntry[].class, cap);
            @SuppressWarnings({"rawtypes","unchecked"})
            MyNode<K,V>[] tab = (MyNode<K,V>[])new MyNode[cap];
            table = tab;
    ...
    abstract class HashIterator {
        MyNode<K,V> next;        // next entry to return
        MyNode<K,V> current;     // current entry
    ...
    static class HashMapSpliterator<K,V> {
        ...
        MyNode<K,V> current;          // current node
    ...
    static final class EntrySpliterator<K,V>
        extends HashMapSpliterator<K,V>
        implements Spliterator<MyMap.MyEntry<K,V>> {
        ...
        public void forEachRemaining(Consumer<? super MyMap.MyEntry<K,V>> action) {
            int i, hi, mc;
            if (action == null)
                throw new NullPointerException();
            MyHashMap<K,V> m = map;
        ...
        public boolean tryAdvance(Consumer<? super MyMap.MyEntry<K,V>> action) {
            int hi;
            if (action == null)
                throw new NullPointerException();
            MyNode<K,V>[] tab = map.table;
    ...
    static final class TreeNode<K,V> extends MyLinkedHashMap.MyEntry<K,V> {
        TreeNode<K,V> parent;  // red-black tree links
        TreeNode<K,V> left;
        TreeNode<K,V> right;
        TreeNode<K,V> prev;    // needed to unlink next upon deletion
        boolean red;
        TreeNode(int hash, K key, V val, MyNode<K,V> next) {
        ...
        static <K,V> void moveRootToFront(MyNode<K,V>[] tab, TreeNode<K,V> root) {
            int n;
        ...
        final TreeNode<K,V> putTreeVal(MyHashMap<K,V> map, MyNode<K,V>[] tab,
                                       int h, K k, V v) {
            Class<?> kc = null;
